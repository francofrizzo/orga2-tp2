...
for imp in $implementaciones; do
    mkdir -p $(dirname $0)/exp2/a/out/blur-$imp
    echo "   Implementación: $imp   Imagen: phoebe1   Sigma: $sigma_fijo"
    printf "%d\n" $repeticiones >> $(dirname $0)/exp2/a-data-blur-$imp.txt
    for r in $radios; do
        printf "%d" "$r" >> $(dirname $0)/exp2/a-data-blur-$imp.txt
        if [ "$verbose" = true ]; then
            echo "  Corriendo instancia r = $r"
        fi
        for k in $(seq $repeticiones); do
            $(dirname $0)/../build/tp2 blur -i $imp -o $(dirname $0)/exp2/a/out/blur-$imp $(dirname $0)/exp2/in/phoebe1-600.bmp $sigma_fijo $r |
                sed -e '/insumidos totales/!d' -e 's/.*: //' |
                while IFS= read -r line; do
                    if [ "$verbose" = true ]; then
                        printf "    Radio: %8s.    Tiempo insumido: %12s\n" "$r" "$line"
                    fi
                    printf " %d" "$line" >> $(dirname $0)/exp2/a-data-blur-$imp.txt
                done
            n1=$($(dirname $0)/../build/tp2 blur -i $imp -n $(dirname $0)/exp2/in/phoebe1-600.bmp)
            n2=$(echo $n1 | sed -e "s/.bmp$/.$sigma_fijo.$r.bmp/")
            # mv $(dirname $0)/exp2/a/out/blur-$imp/$n1 $(dirname $0)/exp2/a/out/blur-$imp/$n2
            rm $(dirname $0)/exp2/a/out/blur-$imp/$n1
        done
        printf "\n" >> $(dirname $0)/exp2/a-data-blur-$imp.txt
    done
done
...
for imp in $implementaciones; do
    mkdir -p $(dirname $0)/exp2/b/out/blur-$imp
    echo "   Implementación: $imp   Imagen: phoebe1   Radio: $r_fijo"
    printf "%d\n" $repeticiones >> $(dirname $0)/exp2/b-data-blur-$imp.txt
    for s in $sigmas; do
        printf "%.2g" "$s" >> $(dirname $0)/exp2/b-data-blur-$imp.txt
        if [ "$verbose" = true ]; then
            echo "  Corriendo instancia sigma = $s"
        fi
        for k in $(seq $repeticiones); do
            $(dirname $0)/../build/tp2 blur -i $imp -o $(dirname $0)/exp2/b/out/blur-$imp $(dirname $0)/exp2/in/phoebe1-600.bmp $s $r_fijo |
                sed -e '/insumidos totales/!d' -e 's/.*: //' |
                while IFS= read -r line; do
                    if [ "$verbose" = true ]; then
                        printf "    Sigma: %8s.    Tiempo insumido: %12s\n" "$s" "$line"
                    fi
                    printf " %d" "$line" >> $(dirname $0)/exp2/b-data-blur-$imp.txt
                done
            n1=$($(dirname $0)/../build/tp2 blur -i $imp -n $(dirname $0)/exp2/in/phoebe1-600.bmp)
            n2=$(echo $n1 | sed -e "s/.bmp$/.$s.$r_fijo.bmp/")
            # mv $(dirname $0)/exp2/b/out/blur-$imp/$n1 $(dirname $0)/exp2/b/out/blur-$imp/$n2
            rm $(dirname $0)/exp2/b/out/blur-$imp/$n1
        done
        printf "\n" >> $(dirname $0)/exp2/b-data-blur-$imp.txt
    done
done
