...
for i in $tamanos; do
    if [ ! -f $(dirname $0)/exp3/in/phoebe1-$i.bmp ]; then
        if [ "$verbose" = true ]; then
            echo "  Generando archivo $(dirname $0)/exp3/in/phoebe1-$i.bmp"
        fi
        convert $(dirname $0)/../img/phoebe1.bmp -scale ${i}x${i} $(dirname $0)/exp3/in/phoebe1-$i.bmp
    fi
    if [ ! -f $(dirname $0)/exp3/in/phoebe2-$i.bmp ]; then
        if [ "$verbose" = true ]; then
            echo "  Generando archivo $(dirname $0)/exp3/in/phoebe2-$i.bmp"
        fi
        convert $(dirname $0)/../img/phoebe2.bmp -scale ${i}x${i} $(dirname $0)/exp3/in/phoebe2-$i.bmp
    fi
done
if [ $? -ne 0 ]; then
...
if [[ $filtros = "diff" || $filtros == "both" ]]; then
    for imp in $imp_diff; do
        mkdir -p $(dirname $0)/exp3/out/diff-$imp
        echo "   Filtro: diff   Implementación: $imp   Imágenes: phoebe1, phoebe2"
        printf "%d\n" $repeticiones >> $(dirname $0)/exp3/data-diff-$imp.txt
        for i in $tamanos; do
            let "j=i*2/3"
            let "t=i*j"
            if [ "$verbose" = true ]; then
            ...
            printf "%d" "$t" >> $(dirname $0)/exp3/data-diff-$imp.txt
            for k in $(seq $repeticiones); do
                $(dirname $0)/../build/tp2 diff -i $imp -o $(dirname $0)/exp3/out/diff-$imp $(dirname $0)/exp3/in/phoebe1-$i.bmp $(dirname $0)/exp3/in/phoebe2-$i.bmp |
                    sed -e '/insumidos totales/!d' -e 's/.*: //' |
                    while IFS= read -r line; do
                        if [ "$verbose" = true ]; then
                            printf "    Tamaño: %8s.    Tiempo insumido: %12s\n" "$t" "$line"
                        fi
                        printf " %d" "$line" >> $(dirname $0)/exp3/data-diff-$imp.txt
                    done
                    nombre=$($(dirname $0)/../build/tp2 diff -i $imp -n $(dirname $0)/exp3/in/phoebe1-$i.bmp $(dirname $0)/exp3/in/phoebe2-$i.bmp)
                    rm $(dirname $0)/exp3/out/diff-$imp/$nombre
            done
            printf "\n" >> $(dirname $0)/exp3/data-diff-$imp.txt
        done
    done
fi
...
if [[ $filtros = "blur" || $filtros == "both" ]]; then
    for imp in $imp_blur; do
        ...
        for i in $tamanos; do
            ...
            printf "%d" "$t" >> $(dirname $0)/exp3/data-blur-$imp.txt
            for k in $(seq $repeticiones); do
                $(dirname $0)/../build/tp2 blur -i $imp -o $(dirname $0)/exp3/out/blur-$imp $(dirname $0)/exp3/in/phoebe1-$i.bmp 5 15 |
                    sed -e '/insumidos totales/!d' -e 's/.*: //' |
                    while IFS= read -r line; do
                        if [ "$verbose" = true ]; then
                            printf "    Tamaño: %8s.    Tiempo insumido: %12s\n" "$t" "$line"
                    ...
                    nombre=$($(dirname $0)/../build/tp2 blur -i $imp -n $(dirname $0)/exp3/in/phoebe1-$i.bmp)
                    rm $(dirname $0)/exp3/out/blur-$imp/$nombre
            done
