...
section .text
    mask_r_to_bgr: db 0x11, 0x01, 0x01, 0x01, 0x11, 0x05, 0x05, 0x05, 0x11, 0x09, 0x09, 0x09, 0x11, 0x0d, 0x0d, 0x0d
    mask_alpha_to_ones: db 0x11, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00
...
diff_asm:
    ...
    ;     punpcklbw xmm3, xmm7
    ;     pcmpgtb xmm2, xmm3  ; xmm2 <- máscara de partes bajas
    ...
    ;     punpckhbw xmm4, xmm7
    ;     pcmpgtb xmm3, xmm4  ; xmm3 <- máscara de partes altas

    ;     packuswb xmm2, xmm3  ; xmm2 <- máscara completa

    ;     movdqu xmm3, xmm0  ; xmm3 <- píxeles de src
    ;     psubb xmm0, xmm1  ; src - src_2
    ;     psubb xmm1, xmm3  ; src_2 - src
    ...
    ;     psrldq xmm1, 8     ; xmm1 =  0 | b1 | g1 | r1 | a1
    ;     pmaxub xmm1, xmm0  ; xmm1 =  x | max(g1, b1) | x | x | ...
    ;     psrldq xmm1, 8     ; xmm1 =  0 | x | max(g1, b1) | x | ...
